...
function ItemComponent({ item }: MyProps) {
  ...
  return (
    <div
      className="card p-[10px] m-[10px] w-[80%] border-2 border-black rounded-lg hover:bg-white"
      key={item.id}
    >
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal}>
        <div className="flex flex-col h-full p-5 justify-between">
          ...
          <div className="flex p-5 justify-end items-end">
            <button
              className="border-black border-2 rounded-md p-1 hover:bg-slate-400"
              onClick={() => goToAothor(`${item.id}`)}
            >
              See more
            </button>
          </div>
